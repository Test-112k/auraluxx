    ...
    >
      {isSearching ? (
        <div className="flex justify-center py-4">
          <div className="w-6 h-6 rounded-full border-2 border-white/10 border-t-aura-purple animate-spin"></div>
        </div>
      ) : results.length > 0 ? (
        <div className="max-h-[400px] overflow-y-auto">
          {results.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="flex items-center p-2 hover:bg-white/5 rounded cursor-pointer transition-colors"
